...
def test_initiate_transfer(db_session, debtor):
    ...
    assert t.transfer_note == {'note': 'test'}
    ...
    assert rt.transfer_note == {'note': 'test'}
    assert not t.is_finalized
...
def test_successful_transfer(db_session, debtor):
    ...
    assert fpts.transfer_note == {'note': 'test'}
    assert fpts.__marshmallow_schema__.dump(fpts)['transfer_note'] == '{"note": "test"}'
